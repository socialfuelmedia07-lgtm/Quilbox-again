...
const StorePage = () => {
    ...
    const transformProducts = (products: any[]) => products.map(p => ({
        ...p,
        id: p._id || p.id,
        ...
        if (storeId) {
            const fetchStoreData = async () => {
                ...
                try {
                    ...
                    const transformedData: StoreProductsResponse = {
                        featured: transformProducts(productsData.featured || []),
                        categories: (productsData.categories || []).map((cat: any) => ({
                            ...cat,
                            products: transformProducts(cat.products || [])
                        }))
                    };
    ...
    const handleSearch = async (query: string) => {
        ...
        setIsSearching(true);
        try {
            const productsData = await storeApi.getStoreProducts(storeId, { search: query });
            const transformedData: StoreProductsResponse = {
                featured: transformProducts(productsData.featured || []),
                categories: (productsData.categories || []).map((cat: any) => ({
                    ...cat,
                    ...
                    <div className="relative rounded-[2rem] overflow-hidden bg-gradient-to-r from-[#fdf2f8] via-[#f5f3ff] to-[#f0f9ff] dark:from-slate-900 dark:to-slate-800 p-8 md:p-12 text-slate-900 dark:text-white shadow-xl border border-white/20">
                        ...
                        <div className="relative z-10 flex flex-col md:flex-row items-center gap-8">
                            ...
                            <div className="flex flex-col gap-2 text-center md:text-left">
                                <h1 className="text-4xl md:text-6xl font-black tracking-tighter leading-tight">{store.name}</h1>
                                <p className="text-slate-500 dark:text-slate-400 text-lg md:text-xl font-bold max-w-2xl leading-snug">{store.description}</p>
                                <div className="flex flex-wrap items-center justify-center md:justify-start gap-4 mt-2">
                                    <span className="text-[#ff3366] font-extrabold uppercase tracking-widest text-xs md:text-sm">Partner Store</span>
                                    <span className="text-slate-300">|</span>
                                    <span className="text-slate-500 font-bold uppercase tracking-widest text-xs md:text-sm">Quick Delivery</span>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                {searchQuery ? (
                    <div className="container mx-auto px-4">
                        <ProductSection
                            ...
                            storeId={storeId}
                        />
                        {data.categories.flatMap(c => c.products).length === 0 && (
                    ...
                    <>
                        ...
                        {/* Priority Sections */}
                        {(() => {
                            const priorityOrder = ["Discounted", "Combo Offer"];
                            const otherCats = data.categories.filter(c => !priorityOrder.includes(c.categoryName) && c.categoryName !== 'Best Seller');
                            const priorityCats = priorityOrder
                                .map(name => data.categories.find(c => c.categoryName === name))
                                .filter((c): c is NonNullable<typeof c> => !!c);

                            return [...priorityCats, ...otherCats].map((cat, idx) => {
                                return (
                                    <ProductSection
                                        key={idx}
                                        ...
                                        subtitle={cat.categoryName === 'Discounted' ? "Grab them before they're gone!" :
                                            cat.categoryName === 'Combo Offer' ? "Big savings on bundles" : ""}
